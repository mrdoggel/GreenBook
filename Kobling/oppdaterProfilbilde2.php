            ...
            if ($filError === 0) {
                if ($filStrl < 1000000) {
                    ...
                    if ($sql->execute() === TRUE) {
                        ...
                        header("location: ../Profil.php?pnr=$pnr&error=".$conn->error);
                    }
                    
                } else {
                    header("location: ../Profil.php?pnr=$pnr&error=Filen_din_er_for_stor");
                }
            } else {
                header("location: ../Profil.php?pnr=$pnr&error=Et_problem_oppstod");
            }
        } else {
            header("location: ../Profil.php?pnr=$pnr&error=Feil_filtype");
        }
    }
